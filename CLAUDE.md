...
4. **Platform variants are separate files.** Don't use
   conditionals inside templates. Each platform (Heroku, Fly, CF
   Pages, Vercel) gets its own complete workflow file.
...
6. **Test against the django-munky example.** The
   `examples/django-heroku-cloudflare/config.json` should always
   reflect what the installer would produce for django-munky's
   3-track setup.
...
4. Verify the installer handles edge cases (existing files,
   missing `gh`, Ctrl+C)
